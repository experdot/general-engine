...
class BlockGroupHelper {
    ...
    static getStandardGroups() {
        return BlockGroupHelper.maps.map(g => new BlockGroup().addOffsetMapRange(g.map(o => new OffsetMap(o.x.map((x, index) => new Vector2(o.x[index], o.y[index]))))));
    }
}
